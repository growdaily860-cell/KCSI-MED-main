...
const first = { provider:'openai', model:'gpt-5.6-luna' };
const second = { provider:'openai', model:'gpt-5-mini' };
...
assert.deepEqual(arena.DEFAULT_OPENAI_MODELS, ['gpt-5.6-luna', 'gpt-5-mini']);

const practiceBody = arena.createRequestBody('gpt-5-mini', 'data:image/jpeg;base64,front', 'data:image/jpeg;base64,back', 'practice');
...
assert.equal(practiceBody.max_completion_tokens, 1200);
assert.equal(practiceBody.max_tokens, undefined);
const researchBody = arena.createRequestBody('gpt-5.6-luna', 'data:image/jpeg;base64,front', '', 'research');
assert.equal(researchBody.messages[0].content[1].image_url.detail, 'high');
assert.equal(researchBody.max_completion_tokens, 2200);
...
const runs = [{
  ...
  blindOrder:{ A:{ provider:'openai', providerLabel:'OpenAI', model:'gpt-5.6-luna' }, B:{ provider:'openai', providerLabel:'OpenAI', model:'gpt-5-mini' } },
  results:{
...
assert(/APP_VERSION = 'v12\.1'/.test(html));
