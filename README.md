...
- 기본 `GPT-5.6 Luna` 대 `GPT-5 mini` 저비용 OpenAI 모델 비교
...
처음 열면 `GPT-5.6 Luna`와 `GPT-5 mini`가 선택됩니다. `저비용 연습`은 각 사진을 OpenAI의 `detail: low`로 전송하고 모델별 최대 출력을 1,200 토큰으로 제한합니다. 비교 1회마다 두 모델을 각각 호출하므로 API 요청은 2회 발생하며, 이미지 입력도 토큰으로 과금됩니다. 작은 각인의 정식 정확도 평가에는 비용이 더 드는 `정밀 비교`를 선택하세요.
