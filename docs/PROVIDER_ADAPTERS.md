...
정적 PWA에서는 Contract·Provider·Runner·Scoring·Report의 동일 소스를 묶은
`research/platform-browser.js`를 `arena.js`보다 먼저 불러온다. `arena.js`의 작은
loader는 번들이 없는 이전 배포판에서 개별 provider 모듈을 순서대로 불러오는
fallback으로 유지한다. `providers/contract.js`는 Task B Contract v1의 브라우저
facade를 노출한다.
...
- `ANTHROPIC_API_KEY` — Anthropic 서버 프록시를 구현·배포할 때 추가
- `GEMINI_API_KEY` — Gemini 서버 프록시를 구현·배포할 때 추가
- `ACCESS_TOKEN`, `LOGIN_PIN`, `REFILL_PIN` — 기존 KCSI 인증·quota 구조 유지

현재 `worker/worker.js`의 운영 upstream은 `/openai`뿐이다. Anthropic/Gemini
Adapter와 Mock은 공통 Contract 및 주입 transport까지 통합되어 있지만, Secret만
추가한다고 실호출이 활성화되지는 않는다. 공통 `/api/research/provider` 프록시의
provider/model allowlist와 동일한 PIN·quota 검증을 Worker에 추가한 뒤 활성화해야 한다.
