  ...
  const METRICS = [
    {
      ...
      formula: '저장된 배치 수. 배치 1건은 알약 1–5개를 모델 4개에 한 번씩 보내므로 모델 응답 4개와 알약별 출력 4–20개를 만든다.',
      meaning: '표본의 크기. 배치가 적으면 아래 모든 비율이 우연히 흔들린다.',
      caution: '배치 크기가 달라질 수 있으므로 배치 수만 보지 말고 시험 알약 N을 함께 봐야 한다. 시험 알약이 25개 미만이면 모델 간 순위를 말하지 않는 편이 낫다.',
      source: 'arena.js · summarizeRuns',
    },
    {
      id: 'cases', group: 'summary', label: '시험 알약',
      formula: '각 저장 배치의 실제 run.cases.length를 합산한다. 사례 배열이 없는 옛 기록만 배치당 5개로 계산해 하위 호환한다.',
    ...
    },
    {
      ...
      formula: '배치 100점 총점의 평균. 총점 = 그 배치의 알약 1–5개 정확성 평균(0~40) + 근거 타당성(0~25) + 환각 억제(0~20) + 명확성(0~15). 알약별 점수는 판정 배점(40/20/0)이 기본이고 조사자가 0~40 사이로 고칠 수 있다.',
