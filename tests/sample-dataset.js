...
async function main() {
  ...
  for (const manifestPath of manifestPaths) {
    ...
    // 외부 unzip 실행 파일에 기대지 않는다. JSZip은 Node/Windows에서도 같은 방식으로
    // entry 목록과 바이트를 읽으므로 기본 PATH만 있는 CI에서도 이 검사가 돌아간다.
    ...
    ['answer_sheet.csv', 'source_manifest.csv', 'README.txt'].forEach(name => {
      assert.ok(zip.file(name), `${where}: ${name}이 없다`);
    });

    // 매니페스트에 적힌 사진 해시가 ZIP 안 실제 사진과 맞는지 (앞 3건만 표본 검사)
    for (const item of manifest.items.slice(0, 3)) {
      for (const side of ['front', 'back']) {
        const entryName = `images/${item.files[side]}`;
        const entry = zip.file(entryName);
        assert.ok(entry, `${where}: ${entryName}이 없다`);
    ...
    const answerCsv = await zip.file('answer_sheet.csv').async('string');
