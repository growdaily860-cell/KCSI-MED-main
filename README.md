...
- 식약처 내장 DB 교차 결과와 정답지 기반 100점 자동평가 제공
- 제품명·각인·신뢰도 보정·응답 완성도 자동 채점과 자동 승자 판정
- 자동평가 후 모델 공개, 누적 통계 및 배치당 20행 연구용 CSV 저장

## AI 모델 비교 연구

상단의 `🧪 모델 비교 연구` 탭에서 익명 배치번호와 알약 5개의 정답지를 입력합니다. 사진 10장은 `1번 앞면, 1번 뒷면, 2번 앞면, 2번 뒷면 … 5번 앞면, 5번 뒷면` 순서로 한꺼번에 선택하거나 각 칸에서 따로 촬영할 수 있습니다. 후보 모델 4개는 실행 시 무작위로 모델 A–D에 배정되며, 자동평가가 끝날 때까지 모델 ID가 숨겨집니다. 각인이 없는 면은 정답에 `없음`이라고 입력합니다.
...
모델 응답은 코드가 자동으로 100점으로 평가합니다. 제품명 판정 40점, 앞·뒤 각인 문자 일치도 25점, 모델 신뢰도의 Brier 보정 15점, 필수 구조화 응답 완성도 20점입니다. 오답인데 높은 신뢰도를 제시하면 Brier 점수가 크게 감점되고, 앞·뒤 사진 순서를 반대로 판독한 경우에는 두 방향 중 더 높은 각인 일치도를 사용합니다. 최고점 모델을 자동 우승으로 저장하며 1점 이내는 동률입니다. 상세 산식과 오픈소스 Promptfoo 연동 방법은 [`docs/MODEL_EVALUATION.md`](docs/MODEL_EVALUATION.md)를 참고하세요.
...
테스트 항목에는 초기화 순서, DOM 로드, 자동 DB 결과 안전 게이트, 수동 확인 상태, T/1 혼동 각인, 업로드 버튼 범위, 블라인드 무작위 배정, 제품명·각인·Brier 자동평가와 통계·CSV 안전성, PIN·서명 세션·일일 한도·미로그인 차단 및 기존 각인·색상 회귀 사례가 포함됩니다.
...
arena.js                   블라인드 모델 비교·자동평가·통계
evaluation/               Promptfoo용 자동평가 어댑터
