(function initDeidReport(root) {
  ...
  function renderBatch() {
    ...
    stats.innerHTML = [
      ...
      { value: pct(summary.completeRate), label: '완전 비식별화' },
      { value: pct(summary.itemRecall), label: '항목 재현율' },
    ...
    tables.innerHTML = `
      <div class="arena-table-wrap"><table class="arena-table"><thead><tr><th>촬영 조건</th><th>문서</th><th>완전 비식별화</th><th>항목 재현율</th></tr></thead><tbody>${
        summary.conditions.map(item => `<tr><td>${esc(item.condition)}</td><td>${item.docs}</td><td>${item.complete} (${pct(item.completeRate)})</td><td>${pct(item.itemRecall)}</td></tr>`).join('')}</tbody></table></div>
      <div class="arena-table-wrap" style="margin-top:10px"><table class="arena-table"><thead><tr><th>개인정보 항목</th><th>개수</th><th>가려짐</th><th>재현율</th></tr></thead><tbody>${
        summary.types.map(item => `<tr><td>${esc(item.type)}</td><td>${item.items}</td><td>${item.covered}</td><td>${pct(item.recall)}</td></tr>`).join('')}</tbody></table></div>`;
  ...
  root.KCSIDeidReport = { isReportRoute, install, state };
