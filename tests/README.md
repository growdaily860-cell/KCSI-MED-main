...
### doc-log.js / doc-redaction.js / rule-ceiling.js / doc-samples.js
...
### threeui-background.js
threeui 배경(`threeui/`)이 **네트워크 없이 뜰 수 있는 상태인지**. 장식이지만 실패 방식이 고약하다 —
CDN 주소가 남거나 vendor 복사본이 어긋나면 인터넷 되는 자리에서만 멀쩡하고 현장에서만 깨진다.

- 변환된 장면 문서에 외부 URL이 하나도 남지 않는가(폰트 `<link>` 포함)
- 소스가 import하는 `three/addons/*`와 그것들이 다시 부르는 파일까지 `vendor/three`에 있는가
  (없으면 `npm run vendor:three`를 다시 돌리라는 뜻)
- 배경 전용 치환(원본 마케팅 문구 끄기)이 헛돌지 않는가 — 대상이 사라지면 예외
- `sandbox="allow-scripts"` iframe이 모듈을 읽을 수 있도록 dev 서버와 `vercel.json`에 CORS 허용이 있는가
- 복사해 쓰는 MIT 코드의 라이선스 원문이 저장소에 남아 있는가

브라우저 렌더링 자체는 확인하지 않는다 — `npm run serve` 후 `/threeui/demo.html`로 눈으로 본다.
