(function initKcsiArena(root) {
  ...
  function scoreBatchWithRubric(groundTruths, predictions, dbChecks) {
    ...
    return AUTO_RUBRIC.scoreBatch(groundTruths, predictions, { dbChecks });
  }
...
{"cases":[{"case_id":"CASE-1","drug_name":"","drug_code":"","imprint_front":"","imprint_back":"","shape":"","color":"","dosage_form":"","confidence":0,"evidence":"","uncertainty":""}]}`;
  ...
  const state = { images: blankImages(), current: null, runs: readRuns(), dataset: blankDataset(), activeGroundTruth: [] };
  ...
  async function loadDatasetBatch() {
    ...
    try {
      ...
      state.images = pairs;
      state.activeGroundTruth = rows.map(row => ({
        schema_version: '1.0',
        sample_id: row.case_id,
        answer: {
          mfds_item_id: row.mfds_item_id,
          drug_name: row.drug_name,
          front_imprint: row.front_imprint,
          back_imprint: row.back_imprint,
          shape: row.shape,
          color: row.color,
        },
        condition: {
          expected_readable: row.expected_readable,
          light: row.light,
  ...
  function readCases(batchId) {
    return Array.from({ length: CASE_COUNT }, (_, index) => {
      ...
      const imported = state.activeGroundTruth.find(item => safeText(item && item.sample_id).trim() === id) || null;
      const importedAnswer = imported && imported.answer || {};
      const importedCondition = imported && imported.condition || {};
      const answer = {
        mfds_item_id: safeText(importedAnswer.mfds_item_id).trim(),
        drug_name: imported && !safeText(importedAnswer.drug_name).trim()
          && safeText(importedAnswer.mfds_item_id).trim() === truthName ? '' : truthName,
        front_imprint: truthFront,
        back_imprint: truthBack,
        shape: safeText(importedAnswer.shape).trim(),
        color: safeText(importedAnswer.color).trim(),
      };
      return {
        schema_version: '1.0',
        sample_id: id,
        ...
        expectedReadable: importedCondition.expected_readable,
        answer,
  ...
  function resetExperiment() {
    state.current = null; state.activeGroundTruth = []; clearAllImages();
