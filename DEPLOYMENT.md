...
- `/`가 정상적으로 열리고 화면에 `v12.12`가 표시되는지 확인
- 강력 새로고침 후 연구 화면이 새 `arena.js` 응답으로 열리고 이전 화면 코드가 남지 않는지 확인
...
- 연구 모드에서 `알약번호`·`파일1/2`·`각인1/2` 머리글 CSV가 인식되고 파생 `PILL-001` 시험번호가 표시되는지 확인
- 폴더가 든 ZIP의 이미지는 basename으로 연결되고 디렉터리·운영체제 메타데이터는 제외되는지 확인
- 큰 ZIP을 선택한 직후 전체 사진을 해제하지 않고, 선택한 1–5건 배치를 불러올 때 필요한 사진만 처리하는지 확인
- EXIF 회전 정보가 있는 세로 사진이 미리보기와 모델 전송용 픽셀에서 같은 방향인지 확인
- 뒷면 파일·각인이 모두 빈 한 면 행이 검증을 통과하고 미제공 면 없이 실행되는지 확인
- 각인 전용 정답에서 `(없음)`·`(마크)`·`(확인불가)`가 서로 구분되고 약물 Top-1 오답으로 집계되지 않는지 확인
- 유효 행이 5의 배수가 아닐 때 마지막 1–4건 배치를 선택·실행할 수 있는지 확인
- 전수 비교 계획의 배치 수와 API 호출 수가 `ceil(유효 행/5) × 4`인지 확인하고, 계획 표시만으로 자동 실행·자동 재개되지 않는지 확인
- 연구 모드에서 식약처 고정 샘플 20건·사진 40장이 자동 검증되고 5건씩 선택되는지 확인
- GPT-4o 이상 4개 모델이 A–D에 배정되고 투표 전 비공개, 투표 후 공개되는지 확인
- 원본 연구 결과 CSV가 배치당 `실제 알약 수 × 4모델`인 4–20행으로 다운로드되는지 확인
- 원본 연구 결과 CSV에 알약번호·제공 면·이미지 참조·분할선·정답 모드·각인 비교 감사 열이 포함되는지 확인
