...
function buildPdfReportHtml(dataset, options = {}) {
  ...
  <h2>연구 개요</h2><p>공통 Contract v1 ResearchResult와 GroundTruth를 기준으로 약물 정답은 Top-1 식별 성능으로, 각인 전용 정답은 각인 정확도·부분정답·CER·지어낸 각인으로 분리해 비교한 보고서입니다. 원본 이미지와 개인정보는 포함하지 않습니다.</p>
  ...
  <h2>모델별 성능표 — 식별·각인</h2><table><thead><tr><th>Provider</th><th>Model</th><th>전체 N</th><th>약물 N</th><th>약물 Top-1</th><th>각인 N</th><th>각인 정확도</th><th>각인 부분정답</th><th>각인 CER</th><th>채점 면</th><th>지어낸 각인</th></tr></thead><tbody>${modelRows || '<tr><td colspan="11">데이터 없음</td></tr>'}</tbody></table>
