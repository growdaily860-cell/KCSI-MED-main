...
- OpenAI·Gemini·Qwen 및 OpenAI 호환 Vision 모델 비교
...
OpenAI 후보는 API 키와 별도 URL을 모두 비우면 기존 KCSI OpenAI Worker 설정을 재사용합니다. Gemini·Qwen 후보는 각 제공자의 API 키를 직접 입력하거나 OpenAI 호환 `/chat/completions` 요청을 받는 전용 Cloudflare Worker 주소를 입력해야 합니다. 입력한 API 키와 Worker 토큰은 연구기록·CSV에 저장되지 않습니다.
