...
- 알약 5개 앞·뒷면 사진 10장 일괄 등록 및 개별 촬영, 데이터셋 배치는 1–5건 지원
...
- CSV·TSV·XLSX·XLS·텍스트/스캔 PDF 정답지와 개별 사진 또는 폴더형 ZIP 데이터셋 업로드
- XLSX 정답지 템플릿과 PDF.js·Tesseract.js 브라우저 로컬 OCR 검토·수정
- `알약번호`·`파일1/2`·`각인1/2` 등 한국어 CSV 머리글 인식과 파생 시험번호 생성
- 시험번호·필수 정답·이미지 파일명 중복/누락·단면 항목 자동 검증 및 행별 오류표
- ZIP 내부 경로를 NFC basename으로 지연 매칭하고 선택한 배치 사진만 메모리에 해제
- EXIF 방향을 픽셀에 반영한 뒤 한 번만 축소해 모든 후보 모델에 동일 입력 사용
- 검증된 데이터셋을 1–5건씩 불러오고 마지막 1–4건 배치도 보존
- 전체 행 수에 따른 예상 배치 수·4모델 API 호출 수와 기본 일일 한도 초과 경고
- 각인 전용 정답 모드와 `(없음)`·`(마크)`·`(확인불가)` 구분, 판정 가능한 제공 면만 채점
- 식약처 공식 등록사진 고정 샘플 20건·앞뒷면 40장 자동 불러오기 및 ZIP 저장
- 연습용 `detail: low`와 정식 평가용 `detail: high` 비용 모드
- 식약처 내장 DB 교차 결과와 100점 평가표 제공
- 투표 후 모델 공개, 누적 통계 및 실제 배치 크기별 4–20행 연구용 CSV 저장
...
`/research`의 `데이터셋 검증` 화면에서는 CSV·TSV·Excel(XLSX/XLS)·PDF 정답지와 알약 사진 여러 장 또는 사진이 든 ZIP 한 개를 선택할 수 있습니다. `case_id` 계열 외에 `알약번호`, `파일1`·`파일2`, `각인1`·`각인2`, `분할선` 같은 한국어 머리글을 인식하며, `case_id`가 없고 `알약번호`가 있으면 `PILL-001` 형식의 재현 가능한 시험번호를 만듭니다. 정답지는 브라우저 안에서만 파싱하고 필수 정답, 이미지 파일명, 중복 및 누락을 행별로 검증합니다. PDF 텍스트 표가 없으면 PDF.js와 Tesseract.js 로컬 OCR로 전환하며, 페이지 원문과 변환 표를 직접 수정·확인하기 전에는 비교 배치로 불러올 수 없습니다. CSV와 2시트 XLSX 템플릿은 화면에서 바로 내려받을 수 있습니다.

ZIP 안의 폴더 경로와 운영체제 메타데이터는 이미지 정답으로 취급하지 않습니다. 이미지 이름은 Unicode NFC basename으로 대조하고 대소문자를 무시하되, 같은 basename이 둘 이상이면 임의로 고르지 않고 오류로 표시합니다. ZIP을 선택할 때는 목록과 크기만 검증하고, 실제 이미지 바이트는 사용자가 1–5건 배치를 불러올 때 필요한 항목만 해제합니다. 선택된 사진은 EXIF-aware 디코딩 결과를 캔버스 픽셀에 반영한 뒤 종횡비를 유지해 한 번만 축소하며, 이 동일한 결과가 네 후보 모델에 전달됩니다.

검증된 행은 5건 단위로 나누고 마지막에 남은 1–4건도 별도 배치로 선택할 수 있습니다. 앞면은 필수지만 정답지에서 뒷면 파일과 뒷면 각인이 모두 비어 있으면 한 면 데이터로 인정하며, 모델에는 미제공 면을 추측하지 말라는 안내와 제공된 면만 전달합니다. 화면의 `전수 비교` 문구는 `ceil(유효 행/5) × 4모델`로 계산한 배치·API 호출 계획과 한도 경고입니다. 각 배치는 사용자가 직접 선택해 불러오고 실행해야 하며, 전체 데이터셋 자동 실행·자동 다음 배치·중단 후 재개 기능을 뜻하지 않습니다.
...
비교 결과는 이 브라우저의 `localStorage`에 최대 100배치 보존합니다. 원본 이미지와 API 인증정보는 누적 연구기록에 포함하지 않습니다. 의약품명이나 품목 ID가 없는 각인 정답지는 약물 Top-1 오답으로 세지 않고 각인 전용 모드로 평가합니다. 이때 `(없음)`은 무각인, `(마크)`는 로고, `(확인불가)`와 빈 정답 면은 평가 제외로 서로 구분하며, 단면 알약은 제공된 면만 분모에 넣습니다. 원본 배치 CSV는 실제 배치의 `1–5개 알약 × 4개 모델 = 4–20행`을 생성하며, 알약번호, 제공 면, 원본 이미지 참조, 분할선 정답, 정답 모드, 각인 유사도·앞뒤 방향, 없는 각인 생성 여부를 함께 기록합니다. Contract v1 자동채점 화면에서는 표준 CSV, Summary·Model Comparison·Per Sample·Errors·Robustness·Cost 6시트 XLSX, PDF 인쇄용 보고서를 추가로 저장할 수 있습니다.
...
deident/batch-runner.js    합성 문서 배치 측정 실행기
deident/report-ui.js       /deid-report 성능 보고 화면
